...
class RightTriangle(GraphScene):
    ...
    def construct(self):
        ...
        lineA = Line(dotC.get_center(), dotA.get_center(), color=RED)
        lineB = Line(dotB.get_center(), dotC.get_center(), color=BLUE)
        lineC = Line(dotA.get_center(), dotB.get_center(), color=PURPLE)

        right_angle = RightAngle(lineB, lineA, length=0.4, quadrant=(-1, 1))
        theta_angle = math.asin(lineB.get_length() / lineC.get_length())

        theta = Arc(
            radius=1,
            start_angle=lineC.get_angle(),
            angle=theta_angle,
            arc_center=lineC.get_start(),
        )
        ...
        a_text = Text(f"a = {a}")
        a_text.next_to(lineA, DOWN * 4)

        b_text = Text(f"b = {b}")
        b_text.next_to(lineB, LEFT * 4)

        c_text = Text(f"c = {c:.2f}")
        c_text.move_to(lineC, UP)
        # c_text.next_to(lineC, LEFT)
...
class PolarCartesian(GraphScene):
    ...
    def construct(self):
        ...
        b2 = Brace(
            line2,
            buff=0.5,
            direction=line2.copy().rotate(line3.get_angle()).get_unit_vector(),
        )
        b3 = Brace(
            line3,
            buff=0.5,
            direction=line3.copy().rotate(line3.get_angle()).get_unit_vector(),
        )
